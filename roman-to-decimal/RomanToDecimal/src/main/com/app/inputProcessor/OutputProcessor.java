...
public class OutputProcessor extends ProcessInput{
	...
	public static void findValueOfRoman(String query){
		if (isValidinput(query)== true){
			...
			for (int i = 0; i < tokenValue.size(); i++) {
				tokenValueToRoman.add(mapOfGalacticValues.get(tokenValue.get(i)));
			}
	...
	private static void findValueOfElement(String query){
		if (isValidinput(query) == true){
			...
			String element = null;
			for (int i = 0; i < tokenValue.size(); i++) {
				if(mapOfGalacticValues.get(tokenValue.get(i)) != null){
					tokenValueToRoman.add(mapOfGalacticValues.get(tokenValue.get(i)));
				}
